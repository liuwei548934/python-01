...
4. 同时进行查询时，也能查询出该用户 。
5. 当用户添加成功的时候，在返回的结果中要显示创建/添加时间
from datetime import datetime
"""
# 实现一个方法：从文件中读取数据，返回的是列表形式的数据
...
# 实现一个方法：向文件中写入内容，就是用户添加的信息是在原来的基础上添加的，不能覆盖原来的
...
# 用户查询功能
  def get_user(self,username):
     # 判断用户名是否登录，若登录成功可以进行查询，若登录失败，返还权限不足
     ...
     lst = [] #此列表存放的是查询到的结果的数据
     for x in user_list:
        account = x.get('account')
        if username in account:   #因为有(in)，所以支持模糊查询
            x.pop('password')
            lst.append(x)
      # 判断列表里的数据，如果列表不为空，代表查询成功，返回对应的结果
     if lst:
...
if __name__ == '__main__':
    ...
    # 1.进行循环，以便用户做各种操作  提供用户的各种选择 比如1 代表登录 2代表查询 q代表退出操作
 flag = True
 while flag:
